    ...
    class MCL_3D: public rclcpp::Node{
        public:
            ...
            : Node("mcl_node", options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_) {
                ...
                // 以下の宣言と代入をコンストラクタに追加
                this->declare_parameter<int>("scanStep", 10);
                ...
                subMultiScan_ = create_subscription<nhk2026_msgs::msg::MultiLaserScan>(
                    "multi_scan", rclcpp::SensorDataQoS(),
                    [this](const nhk2026_msgs::msg::MultiLaserScan::SharedPtr msg) {
                        last_multi_scan_ = msg;
                    }
                );
        ...
        private:
            ...
            void process2DScans(const nhk2026_msgs::msg::MultiLaserScan::SharedPtr& multi_scan, std::vector<Point3D>& points_out) {
                ...
                for (const auto& scan : multi_scan->scans) {
                    ...
                    // 各LiDARの取り付け高さ[m] (ロボットの設計に合わせて調整してください)
                    double mounting_z = 0.15; 
                    if (lidar_id == 0) mounting_z = 0.20; // LD-LiDARは少し高い等
                    ...
                    for (std::size_t i = 0; i < scan.ranges.size(); i += scanStep_) {
                        ...
                        // LD-LiDARかつ実機の場合のオフセット補正
                        if (lidar_id == 0 && !is_sim_) {
                            theta -= 1.5 * M_PI;
                        }
                        ...
                        if (lidar_id == 0) {
                            // LD-Lidar
                            lx = r * cos(theta) + 0.084;
                            ly = r * sin(theta) + 0.013 - 0.013; // もとのコードのママ
                        } 
                        else if (lidar_id == 1) {
                            // Front Lidar (Hokuyo等)
                            lx = r * cos(-1 * theta + M_PI/2) - 0.157;
                            ly = r * sin(-1 * theta + M_PI/2) + 0.2885;
                        } 
                        else if (lidar_id == 2) {
                            // Back Lidar
                            lx = r * cos(theta) + 0.27;
                            ly = r * sin(-1 * theta) - 0.3615;
                        } else {
                            continue;
                        }

                        // 3D点群として格納 (ロボット座標系 base_footprint)
                        Point3D pt;
                        pt.x = lx;
                        pt.y = ly;
                        pt.z = mounting_z; 
            ...
            void pointCloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg) {
                std::vector<Point3D> temp_filtered_points;
                ...
                for (; iter_x != iter_x.end(); ++iter_x, ++iter_y, ++iter_z) {
                    ...
                    temp_filtered_points.push_back(pt);
                }

                // メインループで使うためにメンバ変数にコピー
                last_pc_points_ = temp_filtered_points;
                            // デバッグ用: 何点抽出されたか表示
                            // RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 1000, 
