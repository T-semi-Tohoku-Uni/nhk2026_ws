...
private:
    ...
    void execute(const std::shared_ptr<GoalHandleStepMove> goal_handle) {
        ...
        if(goal->msg == "step up" && count < 2) {
            ...
            if (!send_leg_goal_sync({6.28 + count * 6.28, 6.28 + count * 6.28, 1.57}, goal_handle)) return;
            count++; 
            result->success = true;
            ...
            if (!send_leg_goal_sync({6.1 + count * 6.28, 6.1 + count * 6.28, -1.57}, goal_handle)) return;
            if (!publish_combined_until_lidar(-0.3,-0.5,0.0,0,1,10.0,goal_handle)) return;
            if (!publish_cmd_vel_for_duration(0.0, 0.0, 0.2, goal_handle)) return;
            ...
            if (!publish_cmd_vel_for_duration(0.0, 0.0, 0.2, goal_handle)) return;
            if (!send_leg_goal_sync({0.0 + count * 6.28, 0.0 + count * 6.28, 1.57}, goal_handle)) return;
            result->success = true;
